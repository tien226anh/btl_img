...
export class Chapter3Service {
  ...
  public processImage(file: File, processor_name: string, extra_value: string): Observable<{[key: string]: string}> {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("processor_name", processor_name);
    formData.append("extra_value", extra_value);
    return this.http.post<{ [key: string]: string }>(`${this.baseUrl}chap3/`, formData);
  }
